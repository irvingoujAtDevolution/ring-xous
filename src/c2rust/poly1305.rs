...
len = (len as u64)
.wrapping_sub(16 as std::os::raw::c_int as u64) as size_t
as size_t;
...
*fresh4 = (*fresh4 as std::os::raw::c_uint)
.wrapping_add(
t3 >> 8 as std::os::raw::c_int
| ((1 as std::os::raw::c_int) << 24 as std::os::raw::c_int) as std::os::raw::c_uint,
) as uint32_t as uint32_t;
}
_ => {
if len == 0 {
...
.wrapping_add(t3 >> 8 as std::os::raw::c_int) as uint32_t as uint32_t;
}
}
t[0 as std::os::raw::c_int
as usize] = (mul32x32_64((*state).h0, (*state).r0))
...
as usize] = (mul32x32_64((*state).h0, (*state).r1))
...
as usize] = (mul32x32_64((*state).h0, (*state).r2))
...
as usize] = (mul32x32_64((*state).h0, (*state).r3))
...
as usize] = (mul32x32_64((*state).h0, (*state).r4))
...
(*state)
.h0 = t[0 as std::os::raw::c_int as usize] as uint32_t
& 0x3ffffff as std::os::raw::c_int as std::os::raw::c_uint;
c = t[0 as std::os::raw::c_int as usize] >> 26 as std::os::raw::c_int;
t[1 as std::os::raw::c_int
as usize] = (t[1 as std::os::raw::c_int as usize] as u64).wrapping_add(c)
as uint64_t as uint64_t;
(*state)
.h1 = t[1 as std::os::raw::c_int as usize] as uint32_t
& 0x3ffffff as std::os::raw::c_int as std::os::raw::c_uint;
b = (t[1 as std::os::raw::c_int as usize] >> 26 as std::os::raw::c_int) as uint32_t;
t[2 as std::os::raw::c_int
as usize] = (t[2 as std::os::raw::c_int as usize] as u64)
.wrapping_add(b as u64) as uint64_t as uint64_t;
(*state)
.h2 = t[2 as std::os::raw::c_int as usize] as uint32_t
& 0x3ffffff as std::os::raw::c_int as std::os::raw::c_uint;
b = (t[2 as std::os::raw::c_int as usize] >> 26 as std::os::raw::c_int) as uint32_t;
t[3 as std::os::raw::c_int
as usize] = (t[3 as std::os::raw::c_int as usize] as u64)
.wrapping_add(b as u64) as uint64_t as uint64_t;
(*state)
.h3 = t[3 as std::os::raw::c_int as usize] as uint32_t
& 0x3ffffff as std::os::raw::c_int as std::os::raw::c_uint;
b = (t[3 as std::os::raw::c_int as usize] >> 26 as std::os::raw::c_int) as uint32_t;
t[4 as std::os::raw::c_int
as usize] = (t[4 as std::os::raw::c_int as usize] as u64)
.wrapping_add(b as u64) as uint64_t as uint64_t;
(*state)
.h4 = t[4 as std::os::raw::c_int as usize] as uint32_t
& 0x3ffffff as std::os::raw::c_int as std::os::raw::c_uint;
...
.wrapping_add(b.wrapping_mul(5 as std::os::raw::c_int as std::os::raw::c_uint)) as uint32_t
as uint32_t;
if len >= 16 as std::os::raw::c_int as u64 {
current_block = 8255239616661704268;
} else {
...
let mut state: *mut poly1305_state_st = poly1305_aligned_state(statep);
if (*state).buf_used != 0 {
let mut todo: size_t = (16 as std::os::raw::c_int as u64)
.wrapping_sub((*state).buf_used);
if todo > in_len {
todo = in_len;
}
...
while i < todo {
(*state)
.buf[((*state).buf_used).wrapping_add(i)
as usize] = *in_0.offset(i as isize);
i = i.wrapping_add(1);
}
...
g4 = ((*state).h4)
.wrapping_add(b)
.wrapping_sub(((1 as std::os::raw::c_int) << 26 as std::os::raw::c_int) as std::os::raw::c_uint);
b = (g4 >> 31 as std::os::raw::c_int).wrapping_sub(1 as std::os::raw::c_int as std::os::raw::c_uint);
nb = !b;
...
f0 = (((*state).h0 | (*state).h1 << 26 as std::os::raw::c_int) as u64)
.wrapping_add(
U8TO32_LE(
&mut *((*state).key).as_mut_ptr().offset(0 as std::os::raw::c_int as isize),
) as uint64_t,
);
...
.wrapping_add(
U8TO32_LE(
&mut *((*state).key).as_mut_ptr().offset(4 as std::os::raw::c_int as isize),
) as uint64_t,
);
...
.wrapping_add(
U8TO32_LE(
&mut *((*state).key).as_mut_ptr().offset(8 as std::os::raw::c_int as isize),
) as uint64_t,
);
...
.wrapping_add(
U8TO32_LE(
&mut *((*state).key).as_mut_ptr().offset(12 as std::os::raw::c_int as isize),
) as uint64_t,
);
U32TO8_LE(&mut *mac.offset(0 as std::os::raw::c_int as isize), f0 as uint32_t);
f1 = (f1 as u64).wrapping_add(f0 >> 32 as std::os::raw::c_int) as uint64_t
as uint64_t;
U32TO8_LE(&mut *mac.offset(4 as std::os::raw::c_int as isize), f1 as uint32_t);
f2 = (f2 as u64).wrapping_add(f1 >> 32 as std::os::raw::c_int) as uint64_t
as uint64_t;
U32TO8_LE(&mut *mac.offset(8 as std::os::raw::c_int as isize), f2 as uint32_t);
f3 = (f3 as u64).wrapping_add(f2 >> 32 as std::os::raw::c_int) as uint64_t
as uint64_t;
U32TO8_LE(&mut *mac.offset(12 as std::os::raw::c_int as isize), f3 as uint32_t);
}
