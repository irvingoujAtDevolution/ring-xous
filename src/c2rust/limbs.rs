...
#[no_mangle]
pub unsafe extern "C" fn LIMBS_are_zero(
mut a: *const Limb,
mut num_limbs: size_t,
) -> Limb {
...
#[no_mangle]
pub unsafe extern "C" fn LIMBS_are_even(
mut a: *const Limb,
mut num_limbs: size_t,
) -> Limb {
let mut lo: Limb = 0;
...
let mut dummy: Limb = 0;
let mut lo: Limb = constant_time_is_nonzero_w(
limb_sub(&mut dummy, *a.offset(0 as std::os::raw::c_int as isize), b),
);
...
if num_limbs
.wrapping_rem(
(512 as std::os::raw::c_int as std::os::raw::c_uint).wrapping_div(32 as std::os::raw::c_uint)
as u64,
) != 0 as std::os::raw::c_int as u64
{
return 0 as std::os::raw::c_int;
}
limbs_select(r, table, num_limbs, 32 as std::os::raw::c_int as size_t, index);
